# Ricerca esaustiva (precedentemente vista)
...
f(n)=g(n)+h(n)$$
dove:
- **g(n)** = costo _reale_ del cammino scelto per arrivare fino al nodo `n` (già percorso);
- **h(n)** = stima _euristica_ del costo rimanente per arrivare al goal;
- quindi **f(n)** = stima del costo _totale_ dell'intero cammino dal nodo iniziale al nodo obiettivo, che passa per n.
...
- **g(n)** → può essere uguale o più grande del costo minimo reale `g*(n)`;
...
- ***TEMPO*** -> esponenziale
	...
	- e il numero di questi nodi cresce molto rapidamente, soprattutto se l’euristica non è molto precisa.
		- INFATTI in "esplorati" vengono messi TUTTI i nodi esplorati, non solo quelli che fanno poi parte della soluzione effettiva
...
Se abbiamo più euristiche **senza dominazione reciproca** (cioè nessuna è *sempre* ≥ dell’altra ma si alternano), possiamo costruirne una nuova prendendo **il massimo** tra i loro valori: $$h(n) = max(h_{1}(n), h_{2}(n),...,h_{k}(n)$$
