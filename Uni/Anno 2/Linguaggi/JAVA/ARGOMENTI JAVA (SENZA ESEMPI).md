...
In Java tutte le eccezioni derivano dalla classe base **`Throwable`**, che si divide in due grandi rami:
- **`Exception`** → usata per condizioni anomale che il programma può (o dovrebbe) gestire.
- **`Error`** → indica problemi gravi a livello di JVM o sistema, che in genere **non si gestiscono**.

All’interno di `Exception` distinguiamo:
1. **Checked exceptions** (controllate, **compile time**)
    - Devono essere gestite **esplicitamente** con `try-catch` oppure dichiarate con `throws`.
	    - Esempi: `IOException`, `SQLException`.
	    - Se non le gestisci, il compilatore dà errore.
    
2. **Unchecked exceptions** (non controllate, **runtime**)
    - Estendono `RuntimeException`.
    - Sono errori di programmazione che _possono_ capitare ma non sei obbligato a gestirli.
	    - Esempi: `NullPointerException`, `ArrayIndexOutOfBoundsException`.
    - Il compilatore **non ti obbliga** a catturarle.
...
Generalmente dopo aver aperto una risorsa, uso il finally per chiuderla
...
## Generics
In Java i **Generics** sono dei sistemi di parametrizzazione che permette di scrivere **classi, interfacce e metodi parametrici**, cioè che funzionano con _tipi diversi_ senza dover riscrivere il codice per ogni tipo.
	👉 In pratica introducono un **parametro di tipo** (come una variabile, ma che rappresenta un tipo) e garantiscono **sicurezza di tipo** a tempo di compilazione.

I Generics permettono di creare strutture dati e metodi **riutilizzabili e flessibili**, che funzionano con qualsiasi tipo di dato, mantenendo però il controllo sul tipo usato.
>[!success] Uso `Generics` per evitare di utilizzare il cast ogni volta.
...
- Ogni chiave è unica, mentre i valori possono ripetersi.
- Esempi: `HashMap`, `TreeMap`, `LinkedHashMap`.
```java
