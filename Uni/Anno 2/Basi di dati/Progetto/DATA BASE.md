...
```sql
CREATE VIEW VPrenotazioniPerDataOra AS
	SELECT *
	FROM PRENOTAZIONE
...
```sql
CREATE VIEW VPrenotrazioniConCosto AS
    SELECT *
    FROM PRENOTAZIONE
...
```sql 
CREATE VIEW VCorrieriAttivi AS
    SELECT *
...
```sql
CREATE VIEW VClientiConValutazioniAlte AS
...
```sql
CREATE VIEW VPrenotazioniConDettagliCompleti AS
...
```sql
CREATE VIEW VSediConOrariDisponibili AS
	SELECT *
	FROM SEDE_AMA AS S
	WHERE EXISTS(
	    SELECT *
