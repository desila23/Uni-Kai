...
###### Con $P$ pacchetti 
$$d_{end-to-end} = (N + P - 1) \frac L R$$dove il $-1$ lo mettiamo perché il primo arriva prima degli altri senza intoppi (quindi nella formula non lo contiamo), mentre gli altri saranno vincolati sempre dal primo.
...
- se il traffico non è interessato (sto usando solo io la connessione) allora $$throughput \approx min\{R_j\}$$
	dove $R_{j}$ è la velocità di trasmissione dell'j-esimo collegamento
